LANGUAGE_NAME := tree-sitter-eu4mod
HOMEPAGE_URL := https://github.com/tree-sitter/tree-sitter-eu4mod
VERSION := 0.1.0
DESCRIPTION := Tree-sitter grammar for EU4 modding, covering txt, yml, gfx, and gui files
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/eu4mod '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/eu4mod
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/eu4mod
